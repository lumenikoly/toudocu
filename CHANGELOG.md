...
- Добавлен полный сводный каталог «Процессы» для `UC-*` и `FLOW-*`;
  «Пользовательские сценарии» сохранены самостоятельным верхнеуровневым
  разделом. Типизированные каталоги используют канонические URL по стабильным
  ID, а страница use case содержит вкладки «Описание», «Карта», «Проиграть»,
  «Связи».
