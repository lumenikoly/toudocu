...
- Завершённость `UC-*` теперь требует статуса группы `done`, хотя бы одной
  галочки в критериях приёмки и всех отмеченных галочек раздела; дорожная карта,
  статистика и диагностические сообщения используют один расчёт.
- `ProjectReport` сохраняет схему версии 1 и значение
  `completionSource: "use-case-status"`.
- Сохранение вопроса сразу создаёт ожидающую запись очереди без отдельной
  кнопки; вопрос можно изменить или удалить до получения агентом. Отмена
  закрывает пустую форму без срабатывания обязательной проверки.
- Привязка обсуждения распознаёт выделение с браузерными переносами пробелов и
  учитывает выбранное вхождение повторяющегося текста.
- Кнопка общего списка обсуждений перенесена в заголовок всех страниц основного
  `serve`; панель показывает все ветки проекта и разрешает создать новую только
  при наличии текущего канонического документа.
- Панель «Изменений» теперь открывается справа поверх рабочей области с теми же
  размерами, затемнением, управлением фокусом и мобильным режимом, что и в
  Портале.
- Единый diff сохраняет выделение и предлагает копирование текста, копирование
  контекста и допустимую привязку вопроса. Новые строки получают точный
  диапазон, а старые строки изменённого документа — видимую цитату в сообщении
  без нового вида привязки. Знак «+» снова заметен для допустимых строк.
