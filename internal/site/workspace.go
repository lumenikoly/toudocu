...
<dialog class="review-composer review-delete-confirm" data-review-delete-confirm><form method="dialog" data-review-delete-form><header><strong>Удалить обсуждение?</strong></header><p>Будет удалена вся история ветки. Это действие нельзя отменить.</p><footer><button value="cancel">Отмена</button><button type="submit" class="changes-button danger">Удалить</button></footer></form></dialog>
