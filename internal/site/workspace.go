...
{{.Header}}
<section class="workspace-context changes-context" aria-label="Параметры сравнения"><div class="changes-range"><label>Base<input type="text" value="HEAD" data-base spellcheck="false"></label><label>Base branch<input type="text" data-branch-base placeholder="main" spellcheck="false"></label><span aria-hidden="true">→</span><label>Target<select data-target><option value="working-tree">working tree</option><option value="index">index</option><option value="HEAD">HEAD</option><option value="revision">Git revision…</option></select></label><label data-target-revision-wrap hidden>Target revision<input type="text" data-target-revision spellcheck="false" placeholder="abc1234"></label><button type="button" class="changes-button" data-apply-range>Сравнить</button></div><div class="changes-review-actions"><button type="button" class="changes-button secondary" data-mobile-files aria-expanded="false">Файлы</button><button type="button" class="changes-button secondary" data-global-comment>Общий комментарий</button><button type="button" class="changes-button secondary" data-discussions-toggle aria-expanded="false">Обсуждения · <span data-open-discussion-count>0</span></button></div></section>
<main id="changes-main" class="changes-workspace">
  <section class="changes-overview" aria-labelledby="changes-title"><div><h1 id="changes-title">Изменения</h1><p data-range-summary>Подготовка Git-сравнения…</p></div><dl class="changes-metrics" data-summary aria-label="Сводка изменений"></dl></section>
  ...
  <div class="changes-toolbar"><label class="changes-search"><span>Поиск</span><input type="search" data-search placeholder="Путь или название"></label><label><span>Статус</span><select data-status><option value="">Все</option><option value="added">Добавленные</option><option value="untracked">Untracked</option><option value="modified">Изменённые</option><option value="deleted">Удалённые</option><option value="renamed">Переименованные</option></select></label><label><span>Область</span><select data-classification><option value="">Все</option><option value="permanent-documentation">Постоянная документация</option><option value="work-artifact">Рабочие артефакты</option><option value="contract">Контракты</option><option value="asset">Assets</option></select></label><label><span>Git</span><select data-git-state><option value="">Любое состояние</option><option value="staged">Staged</option><option value="unstaged">Unstaged</option><option value="untracked">Untracked</option><option value="committedInBranch">Committed in branch</option></select></label></div>
  <div class="changes-split"><aside class="changes-list-panel" data-files-panel aria-label="Файлы" role="navigation"><div class="changes-list-heading"><h2>Файлы</h2><span data-result-count></span><button type="button" data-files-close aria-label="Закрыть список файлов">×</button></div><div class="changes-file-list" data-file-list></div><button type="button" class="changes-linked-add" data-linked-file-open>+ Связать файл</button></aside><section class="changes-detail" data-detail aria-live="polite"><div class="changes-empty" data-ui-state="empty"><h2>Выберите файл</h2><p>Откройте изменённый или связанный файл слева.</p></div></section><aside class="review-panel" data-discussions-panel aria-label="Обсуждения" role="complementary" hidden><header><div><strong>Обсуждения</strong><span data-review-summary></span></div><button type="button" data-discussions-close aria-label="Закрыть обсуждения">×</button></header><div class="review-thread-list" data-discussion-list></div><footer><button type="button" class="changes-button" data-send-feedback disabled>Отправить агенту · <span data-unsent-count>0</span></button></footer></aside></div>
</main>
<dialog class="review-composer" data-review-composer><form method="dialog" data-review-form><header><div><strong>Новый комментарий</strong><span data-review-target-summary></span></div><button value="cancel" aria-label="Закрыть">×</button></header><label>Тип<select data-review-type><option value="issue">Проблема</option><option value="suggestion">Предложение</option><option value="question">Вопрос</option><option value="praise">Похвала</option></select></label><label>Комментарий<textarea data-review-message rows="4" maxlength="65536" required></textarea></label><p class="form-error" data-review-error role="alert"></p><footer><button value="cancel">Отмена</button><button type="submit" class="changes-button" value="default">Добавить <span class="shortcut-hint">Ctrl/⌘ Enter</span></button></footer></form></dialog>
