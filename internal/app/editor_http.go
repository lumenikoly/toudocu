...
func (s *documentationServer) serveEditorUI(w http.ResponseWriter, r *http.Request) {
	...
	s = &documentationServer{model: uiModel}
	title := escapeHTML(uiModel.Project.Title)
	_, _ = fmt.Fprintf(w, `<!doctype html><html lang="ru"%s><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light dark"><title>Редактор — %s</title><link rel="icon" href="%s">%s<link rel="stylesheet" href="/assets/style.css"><link rel="stylesheet" href="/assets/editor.css"><script src="/assets/codemirror.js" defer></script><script src="/assets/editor.js" defer></script></head><body class="workspace-body editor-body"><a class="editor-skip" href="#editor-workspace">Перейти к редактору</a><div class="editor-shell">%s<section class="workspace-context editor-context" aria-label="Действия редактора"><div class="editor-path"><button type="button" data-tree-toggle aria-label="Открыть дерево файлов"><span aria-hidden="true">☰</span><span>Файлы</span></button><span data-current-path>Выберите документ</span><span class="dirty-mark" data-dirty-state hidden>Изменён</span></div><div class="editor-actions"><a data-raw-link href="#" target="_blank" rel="noopener" hidden>Исходник</a><button type="button" data-create-open>Создать</button><button class="primary" type="button" data-save disabled>Сохранить</button></div></section><div class="editor-layout"><aside class="editor-tree" data-tree><div class="tree-heading"><strong>Документы</strong><button type="button" data-tree-close aria-label="Закрыть дерево">Закрыть</button></div><label class="tree-search"><span>Фильтр</span><input type="search" data-file-filter placeholder="Путь или название"></label><nav aria-label="Исходные файлы" data-file-tree></nav></aside><main id="editor-workspace" class="editor-workspace"><div class="editor-notice" data-conflict hidden role="alert"><strong>Файл изменён снаружи.</strong><span data-conflict-message>Ваш текст сохранён в редакторе. Сравните версии или подтвердите перезапись.</span><button type="button" data-conflict-load>Загрузить внешнюю</button><button type="button" data-conflict-overwrite>Перезаписать</button><button type="button" data-conflict-download hidden>Скачать текст</button></div><div class="editor-tabs" role="tablist" aria-label="Режим просмотра"><button role="tab" aria-selected="true" data-view="editor">Редактор</button><button role="tab" aria-selected="false" data-view="preview">Preview</button><button role="tab" aria-selected="false" data-view="split">Разделённый экран</button></div><div class="editor-stage" data-stage="editor"><section class="source-pane" aria-label="Редактор исходного текста"><div data-editor-host></div><textarea data-editor-fallback spellcheck="false" aria-label="Содержимое документа"></textarea></section><section class="preview-pane" aria-label="Preview" data-preview><div class="preview-empty">Preview доступен для Markdown.</div></section></div><section class="diagnostics" aria-labelledby="diagnostics-title"><div class="diagnostics-heading"><strong id="diagnostics-title">Diagnostics</strong><span data-diagnostic-count>0</span></div><ol data-diagnostics></ol></section></main></div></div><dialog data-create-dialog><form method="dialog" data-create-form><header><h2>Создать документ</h2><button value="cancel" aria-label="Закрыть">Закрыть</button></header><label>Шаблон<select data-template-select></select></label><label>Язык<select data-template-language></select></label><div data-template-fields></div><p class="form-error" data-create-error role="alert"></p><footer><button value="cancel">Отмена</button><button class="primary" type="submit" value="default">Создать</button></footer></form></dialog><div class="editor-toast" data-toast role="status" aria-live="polite"></div></body></html>`, appearanceAttributes(s.model.SiteConfig), title, escapeAttr(workspaceFavicon(s.model)), appearanceEarlyScript(s.model.SiteConfig), workspaceHeader(s.model, workspaceEditor))
}
