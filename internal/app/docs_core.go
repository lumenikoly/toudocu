...
func validateDocumentBasics(model *Model, document *Document) {
	...
	rules := map[string][]sectionRule{
		"use-case": {
			{[]string{"основной сценарий", "main scenario", "main flow", "основной поток"}, "Отсутствует раздел «Основной сценарий»."},
		...
		"module": {
			{[]string{"расположение в коде", "code location", "code locations"}, "Отсутствует раздел «Расположение в коде»."},
			{[]string{"границы", "границы модуля", "boundaries", "module boundaries"}, "Отсутствует раздел «Границы»."},
			{[]string{"бизнес-правила", "business rules"}, "Отсутствует раздел «Бизнес-правила»."},
