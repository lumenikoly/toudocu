...
func runSkillCLI(argv []string, stdin io.Reader, stdout, stderr io.Writer, interactive bool) int {
	...
	cwd, err := os.Getwd()
	if err != nil {
		_, _ = fmt.Fprintln(stderr, "SKILL_ROOT_UNAVAILABLE:", err)
		return 1
	}
	...
	if err != nil {
		_, _ = fmt.Fprintln(stderr, "SKILL_HOME_UNAVAILABLE:", err)
		return 1
	}
	agent := options.agent
	if agent == "auto" {
		...
		if len(detected) == 1 {
			...
			if err != nil {
				_, _ = fmt.Fprintln(stderr, "SKILL_AGENT_REQUIRED:", err)
				return 1
			}
		} else {
			_, _ = fmt.Fprintln(stderr, "SKILL_AGENT_REQUIRED: --agent is required when auto-detection finds zero or multiple hosts")
			return 1
		}
	}
...
func chooseAgent(input io.Reader, output io.Writer, detected []string) (string, error) {
	_, _ = fmt.Fprintln(output, "Select an AI host:")
