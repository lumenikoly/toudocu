...
- Treat the canonical documentation root as the only documentation and backlog
  ...
  Read only the selected translation root for an explicit `$toudocu translate
  <locale>` or an explicit request to check, find, build, run, or inspect that
  locale. For an explicit `$toudocu translate diff`, process all configured
...
- Use `$toudocu refresh` for a full evidence-based review and update of
  source documentation. Use `$toudocu refresh diff` for staged, unstaged,
