...
| Agent feedback HTTP API | Main `serve` only | Store discussion state, queue messages automatically, edit them before retrieval, and show responses | Changes local state outside the repository only; targets are limited to canonical documentation | [OpenAPI](../contracts/agent-feedback.openapi.yaml), [architecture](../architecture/agent-feedback-delivery.md) |
