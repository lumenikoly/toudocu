...
- команды `check`, `task context`, `task check`, `build`, `serve`, `version`;
...
Собирает портал, запускает небольшой сервер для разработки и пересобирает
документацию при обновлении HTML-страницы.

```bash
# Доступ только с текущего компьютера
docgent serve ./docs
...
По умолчанию сервер слушает `127.0.0.1:8080`. При `0.0.0.0` откройте на другом
устройстве `http://<LAN-IP-компьютера>:8080/`. Сервер не предоставляет TLS или
авторизацию и не предназначен для публичной сети.
...
-t, --title <name>             Название проекта
    ...
    --host <address>           Адрес serve; по умолчанию 127.0.0.1
    --port <number>            Порт serve; по умолчанию 8080
...
- Команды из work item считаются доверенным исполняемым кодом и запускаются
  только явным вызовом `task check`; обычные `check`, `task context`, `build` и
  `serve` никогда их не выполняют.
- `serve` раздаёт только сгенерированный output; исходный Markdown и остальные
  файлы репозитория через HTTP недоступны.
- Сетевой режим `serve --host 0.0.0.0` не имеет авторизации и TLS и должен
  использоваться только в доверенной локальной сети.
