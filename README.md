...
Небольшую схему храните в связанном use case или архитектурном документе.
Переиспользуемый процесс можно вынести в `flows/FLOW-*.md` с уникальным
`FLOW-*` и ссылкой на use case либо архитектуру. Диаграмма остаётся
визуализацией: требования определяют use cases, контракты и критерии приёмки.

## Карта экранов

Опциональный `screens/map.md` задаёт машинный каталог экранов и переходы двумя
Markdown-таблицами. Docgent проверяет `SC-*`, модули, маршруты и топологию, а
затем строит автономную Mermaid-карту и каталог с фильтрами.
...
|---|---|---|---|---|
| SC-AUTH-LOGIN | Продолжить | Успех | SC-ACCOUNT-DASHBOARD | redirect |
```

Значимые экраны можно дополнить отдельными `screens/SC-*.md`. Use cases и
задачи ссылаются на них полем `Экраны`. Подробный формат, роли и правила
проверки описаны в [руководстве по карте экранов](docs/guides/screens.md).
...
Чекбоксы разрешены только в разделе `Критерии приёмки`. Каждый критерий должен
начинаться с уникального `AC-*`, а каждая строка раздела `Проверка` — ссылаться
ровно на один `AC-*`, `ALL` или `DOCS` и содержать команду.
...
Команда находит задачу по ID и выводит всё, что нужно для работы над ней:
...
### Выполнить проверки задачи

```bash
docgent task check TASK-DOCS-001 ./docs
```
...
Если описание корректно, Docgent запускает команды `AC-*`, `ALL` и `DOCS` из
...
`blocked`. Команда `task check` завершается с кодом `0` только при статусе
`passed`.
...
пользователя. Запускайте `task check` только в доверенном репозитории. Обычные
`check`, `build`, `serve` и `task context` эти команды никогда не выполняют.

## Что получается на выходе

Сборка создаёт автономный HTML-портал с навигацией, поиском, обратными ссылками,
Mermaid-диаграммами, тёмной темой и печатной версией. Рядом сохраняется
`report.json` с результатами проверки. Его можно читать в CI или других
инструментах.
