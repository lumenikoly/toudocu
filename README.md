...
- проверяемые задачи с локальными `AC-*`, командами проверки и матрицей в `report.json`;
...
Общий прогресс проекта вычисляется только по чек-листам корневого `roadmap.md`. Каждый элемент roadmap обязан содержать стабильный ID существующего use case или контракта либо уникальный `DLV-*`. `status.md` не может содержать собственный чек-лист требований.
...
Чекбоксы вне раздела критериев запрещены. Для `Выполнено` все критерии должны быть отмечены; `Заблокировано` требует раздел `Блокер`, а `Отменено` — `Причина отмены`.
