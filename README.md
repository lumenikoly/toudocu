...
`http://127.0.0.1:8080`. Пока команда работает, Docgent пересобирает сайт при
каждом запросе HTML-страницы. Для явного обновления уже открытой страницы
нажмите в хедере кнопку «Пересобрать документацию»: Docgent перегенерирует HTML
и перезагрузит страницу, сохранив адрес сервера. Остановить сервер можно
сочетанием `Ctrl+C`.

## Что должно быть в `docs`

Глобально требуется только `index.md`. Опциональный `quality/` содержит
...
Кнопка ручной пересборки отображается только после подтверждения endpoint
`serve` и отсутствует при просмотре через `file://` или другой статический
HTTP-сервер.
...
при необходимости создаёт минимальный `docs/index.md`, а затем добавляет или
обновляет ограниченный управляемый блок в корневом `AGENTS.md`. Обычные вызовы
skill не устанавливают project instructions автоматически; команды
`docgent init` в Go CLI нет.
