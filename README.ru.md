...
Toudocu не запускает агента. Сохранение сообщения сразу создаёт запись очереди;
сообщение можно изменить или удалить, пока агент не получил запрос. Кнопка «Скопировать
промт» мгновенно копирует просьбу обработать очередь. Установленный навык
получает по одному запросу через `toudocu agent next --json` и возвращает ответ
командой `toudocu agent respond` в исходное обсуждение.
