...
sequenceDiagram
    ...
    participant Changes as Раздел «Изменения»
    ...
    Review-->>Portal: Подтвердить сохранение
    User->>Changes: Открыть обсуждение выбранного файла
    User->>Changes: Добавить комментарии и нажать «Отправить агенту»
    Changes->>Review: Зафиксировать все новые сообщения открытых обсуждений
    ...
    Review-->>Changes: Показать ответы в исходных обсуждениях
    User->>Changes: Закрыть обсуждение или ответить ещё раз
```
