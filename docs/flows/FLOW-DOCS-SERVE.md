...
flowchart TD
    ...
    Request --> Manual{"Запрошена ручная пересборка?"}
    ...
    Reload --> Request
    ManualError --> Request
    Manual -->|Нет| HTML{"Запрошена HTML-страница?"}
...
- Ручная пересборка обновляет модель и HTML, но не закрывает listener и не
  меняет его адрес.
- Кнопка ручной пересборки появляется только после подтверждения служебного
  endpoint `serve`; статический портал через `file://` или другой HTTP-сервер
  остаётся без этого действия.
