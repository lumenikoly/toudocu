...
Поле `Сценарий` поддерживает несколько `UC-*`. Toudocu строит обе стороны
...
безопасного стабильного ID Toudocu создаёт:
...
Весь интерфейс остаётся read-only и работает на HTTP(S) static hosting без
Toudocu backend.
