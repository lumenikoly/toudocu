...
в тех же обсуждениях страницы документа и раздела «Изменения».
...
- `web/src/core/portal.ts` и `web/src/styles/serve.css` — действия с выделенным
  текстом и панель обсуждений на странице документа;
...
удалить. После передачи сообщение и пакет нельзя редактировать; продолжение
обсуждения создаётся новым ответом. Отдельное удаление всей ветки уничтожает её
историю. Затронутый ожидающий пакет при этом отменяется целиком, а не
переписывается.
