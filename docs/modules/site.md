...
Только canonical `serve` включает capability проверки версии. По первому
same-origin запросу Go один раз получает метаданные latest stable release из
фиксированного GitHub endpoint, применяет timeout и предел ответа и кеширует
результат до остановки процесса. Более новая версия показывается как
неблокирующее предложение открыть официальный релиз; скрытие действует только
для этой версии. Любая ошибка остаётся silent. `--no-update-check`, static
build, locale mounts и direct translation serve оставляют capability
выключенной, endpoint недоступным и не выполняют проверку.
...
- строка текущего состояния существует только при наличии status, roadmap,
  work items или рисков, называет статус и ближайший результат и выбирает один
  переход в порядке `status.md` → документ результата → каталог работ → риски;
  счётчики и подробные элементы остаются на статусной странице и в каталогах;
...
- update notice сохраняется между мягкими переходами, не показывается повторно
  для скрытой версии и никогда не блокирует основной content;
