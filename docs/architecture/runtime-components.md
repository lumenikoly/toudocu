...
обсуждениями, очередью пакетов и переносом привязок, а `ReviewStore` безопасно
...
| Обсуждения | Хранит локальные обсуждения и передаёт неизменяемые пакеты агенту | [MOD-REVIEW](../modules/MOD-REVIEW.md) |
| Встроенный skill | Проверяет пакет, выбирает место установки и безопасно выполняет `install`, `update` или `uninstall` | [Руководство](../guides/skill-installation.md) |
...
Установка skill — отдельная короткая ветка CLI: она не строит модель
