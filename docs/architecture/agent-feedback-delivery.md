...
flowchart LR
    ...
    Skill -->|читает; меняет в границах цели file или document| Sources[Безопасные файлы репозитория]
...
изменение. Для `document` агент ограничен канонической документацией, а для
`file` может изменить связанные безопасные пути репозитория. Наблюдатель и
пересборка перечитывают реальные файлы.
