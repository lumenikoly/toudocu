...
Для комментария браузер передаёт вид привязки и координаты. Go сам проверяет
путь, извлекает выделенный текст с контекстом и определяет новое положение после
изменения файла.

Исходники интерфейса находятся в `web/`, проверяются в строгом режиме
TypeScript и собираются esbuild. Собранные ресурсы хранятся в
`internal/site/assets/generated/` и встраиваются в бинарник. Node.js нужен
только при разработке интерфейса; готовому бинарнику и обычной сборке Go он не
нужен.
