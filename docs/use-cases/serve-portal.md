...
1. Разработчик вызывает `toudocu serve ./docs`.
2. Toudocu собирает портал в выходной каталог.
3. Toudocu запускает HTTP-сервер на `127.0.0.1:8080` и сообщает адрес.
...
6. Если разработчик переходит в `/_toudocu/api-docs/`, он выбирает Editor или
...
7. Иначе на странице roadmap разработчик может выбрать существующий этап,
   ...
   результат. Toudocu выполняет CAS insertion и возвращает страницу к этапу.
8. Или разработчик открывает `/_toudocu/editor/`; Editor получает revision,
...
10. Toudocu сравнивает SHA-256 digest, атомарно заменяет файл и синхронно
...
Locale mount `/_toudocu/locales/<locale>/` является read-only: он не содержит
