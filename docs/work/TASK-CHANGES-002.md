...
- вопросы и запросы на изменение относятся только к канонической документации;
  сохранение сразу создаёт редактируемую до получения агентом запись очереди,
  а «Скопировать промт» не меняет состояние и не запускает агента.
