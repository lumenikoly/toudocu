...
- интерпретация natural-language запроса внутри Docgent;
...
- `ALL` → `go test ./...`
- `DOCS` → `go run ./cmd/docgent check ./docs --repository-root . --strict`
...
`docs/decisions/ADR-002.md` и `skills/use-docgent/`.
