...
- [x] `AC-04` Обсуждение позволяет создать ветку, ответить, изменить или
  удалить неотправленное сообщение, закрыть, открыть заново, удалить любую
  ветку и очистить старое состояние. Отправленное сообщение нельзя
  редактировать.
- [x] `AC-05` Отправленные сообщения и записи очереди неизменяемы и выдаются по
  порядку поступления. До ответа команда не переходит к следующей записи, а
  после истечения срока блокировки повторно выдаёт ту же; повтор идентичного
  ответа идемпотентен.
