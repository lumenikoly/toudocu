...
передают новые комментарии установленному AI-skill через CLI schema v1. Toudocu
...
- [x] `AC-12` На странице документа выделение открывает действия копирования
  ...
  привязку либо безопасную привязку ко всему файлу. Панель на той же странице
  позволяет ответить, закрыть, открыть, удалить и передать ветку агенту.
...
- `AC-12` → `TR-SITE-007` → `Portal and Changes review`
...
адаптивная панель там же показывает ответы, управляет состоянием и удалением
веток и готовит новые сообщения для агента без перехода в «Изменения».
