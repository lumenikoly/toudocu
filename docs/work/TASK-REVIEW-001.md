...
Сообщение имеет назначение `question` или `change_request`. «Добавить в очередь»
создаёт постоянную запись `AgentDelivery`, а «Скопировать промт» мгновенно копирует
фразу «Обработай запросы из Toudocu.» без изменения состояния. Агент отдельно
выполняет `toudocu agent next|respond`. На странице документа вопрос можно
создать из выделенного фрагмента; адаптивная панель там же показывает ответы,
управляет черновиками, состоянием и удалением веток без перехода в Changes.
