...
При просмотре через `serve` хедер проверяет служебный endpoint и показывает
кнопку ручной пересборки. Она повторно строит модель и HTML, затем перезагружает
текущую страницу, не останавливая listener и не меняя адрес. Через `file://`
или другой статический HTTP-сервер кнопка остаётся скрытой.
