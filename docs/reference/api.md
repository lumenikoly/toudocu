...
| HTTP API обратной связи с агентом | Только основной `serve` и рабочий Git diff | Состояние обсуждения, автоматическая постановка сообщения в очередь, редактирование до получения агентом и ответы | Меняет только локальное состояние вне репозитория; цели `document` и `file` ограничены безопасными путями | [OpenAPI](../contracts/agent-feedback.openapi.yaml), [архитектура](../architecture/agent-feedback-delivery.md) |
