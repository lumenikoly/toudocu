...
| HTTP API обсуждений | Только основной `serve` | Список допустимых файлов всего репозитория, обсуждения и ответы агента | Любой диапазон читается; запись только для `working-tree`; состояние лежит вне репозитория | [OpenAPI](../contracts/changes.openapi.yaml), [поведение](../contracts/changes-http.md#обсуждения-всего-репозитория) |
| CLI передачи комментариев | Бинарник или исходный репозиторий | Передача ожидающих пакетов между Changes и skill | Читает и пишет только локальное состояние обсуждений; агента и Git не запускает | [CLI-контракт](../contracts/cli.md#ответ-агента-на-комментарии) |
| Справка HTTP API | `/_toudocu/api-docs/` только в основном `serve` | Просмотр обоих OpenAPI-контрактов и безопасные пробные запросы | Только тот же origin; из интерфейса разрешены `GET` и `HEAD`; CDN нет | [Экран](../screens/SC-SITE-API-DOCS.md) |
