...
`task changes TASK-ID --tree` доступна только для `TASK-*` и объединяет
выбранную задачу и всех её потомков; `BUG-*` в дерево не входит. Каждый
заявленный путь содержит `declaredBy` с ID задач-владельцев. Markdown самих
задач перечисляется в `taskChanges` и не считается постоянной документацией.
