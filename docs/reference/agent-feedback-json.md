...
Для документа сервер вычисляет строки и столбцы в символах Unicode, если
фрагмент найден в текущем Markdown. Если текст или его порядковый номер не
совпали, сервер сохраняет `selectedText`, но создаёт обсуждение без диапазона на
уровне документа. Координаты элементов страницы не сохраняются. Для файлов в
разделе изменений выделение остаётся строгим: несовпавший фрагмент отклоняется.

Поле `target.kind` принимает `document` и `file`. Цель `file` создаётся только
для файла рабочего diff. Если доступен UTF-8 текст размером не более 2 МиБ, цель
может содержать диапазон; иначе обсуждение относится ко всему файлу. Удалённая
цель имеет состояние `deleted`, но существующая ветка остаётся доступной.
...
`needs_clarification` или `failed`. `changed` требует хотя бы один безопасный
путь: для `document` — внутри канонической документации, для `file` — внутри
репозитория. Для `question` результат `changed` запрещён.
